            ...
            <div class="px-6 pb-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="rounded-xl border border-gray-200 bg-green-50 p-4 dark:border-gray-700 dark:bg-green-500/10">
                        ...
                        <div class="mt-1 font-bold text-green-600 text-title-sm dark:text-green-500">
                            {{ number_format($transactions->where('transaction_type', 'credit')->sum('amount'), 2) }} BDT
                        </div>
                    </div>
                    <div class="rounded-xl border border-gray-200 bg-red-50 p-4 dark:border-gray-700 dark:bg-red-500/10">
                        ...
                        <div class="mt-1 font-bold text-red-600 text-title-sm dark:text-red-500">
                            {{ number_format($transactions->where('transaction_type', 'debit')->sum('amount'), 2) }} BDT
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-gray-200 border-y dark:border-gray-700">
                                ...
                                <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Account</th>
                                <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Type</th>
                                <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Amount</th>
                                <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Description</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($transactions as $transaction)
                                <tr>
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $transaction->transaction_type == 'credit' ? 'bg-green-100 text-green-600 dark:bg-green-500/15 dark:text-green-500' : 'bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-500' }}">
                                            {{ ucfirst($transaction->transaction_type) }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <div class="text-sm font-bold {{ $transaction->transaction_type == 'credit' ? 'text-green-600' : 'text-red-600' }}">
                                            {{ $transaction->transaction_type == 'credit' ? '+' : '-' }}{{ number_format($transaction->amount, 2) }} BDT
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
