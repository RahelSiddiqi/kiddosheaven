...
@section('title', 'Edit Flash Sale — Kiddo\'s Heaven')
...
@section('content')
	...
	<div x-data="{ productModalOpen: false }">
				...
				<form action="{{ route('admin.marketing.flash-sales.update', $flashSale) }}" method="POST" id="flashSaleForm">
					...
					<div class="flex flex-col gap-2 mb-6 sm:flex-row sm:items-center sm:justify-between">
						<div>
							...
							<h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Edit Flash Sale</h1>
						...
						<div class="flex gap-3">
							...
							<button type="submit"
								...
								<svg class="mr-2" width="20" height="20" viewBox="0 0 20 20" fill="none"
									...
									<path d="M2.5 10.8333L7.91667 15.4167C8.375 15.7917 9.125 15.7917 9.58333 15.4167L17.5 9.16667"
					...
					<div class="grid gap-6 lg:grid-cols-12">
						...
						<div class="lg:col-span-8 space-y-6">
							...
							<div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
								...
								<div class="space-y-4">
									...
									<div class="grid gap-4 sm:grid-cols-2">
										<div>
											<label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Flash Sale Name
												*</label>
											<input type="text" id="name" name="name"
												...
												placeholder="e.g., Summer Flash Sale" required value="{{ old('name', $flashSale->name) }}">
											@error('name')
										...
										<div>
											<label for="discount_percentage"
												class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Discount Percentage *</label>
											<div class="relative">
												<input type="number" id="discount_percentage" name="discount_percentage"
													...
													placeholder="10" min="1" max="99" step="0.01" required
							...
							<div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
								...
								<div class="grid gap-4 sm:grid-cols-2">
									<div>
										<label for="starts_at" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Start Date &
											Time *</label>
										<input type="datetime-local" id="starts_at" name="starts_at"
											...
											required
											value="{{ old('starts_at', $flashSale->starts_at ? $flashSale->starts_at->format('Y-m-d\TH:i') : '') }}">
										@error('starts_at')
											<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
										@enderror
									</div>
									<div>
										<label for="ends_at" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">End Date & Time
											*</label>
										<input type="datetime-local" id="ends_at" name="ends_at"
											class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-blue-800"
											required value="{{ old('ends_at', $flashSale->ends_at ? $flashSale->ends_at->format('Y-m-d\TH:i') : '') }}">
										@error('ends_at')
											<p class="text-red-500 text-xs mt-1">{{ $message }}</p>
											@endif
										</div>
									...
									<!-- Tips -->
									<div class="rounded-2xl border border-gray-200 bg-white p-6 my-4 dark:border-gray-800 dark:bg-white/3">
								...
								<div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
									<div class="flex items-center justify-between mb-4">
										...
										<button type="button" @click="productModalOpen = true"
											...
											<svg class="mr-1" width="14" height="14" viewBox="0 0 20 20" fill="none"
												xmlns="http://www.w3.org/2000/svg">
									...
									<div id="selectedProductsContainer">
										...
										@else
											...
											@endif
											<div id="selectedProductsList" class="space-y-2 {{ $flashSale->products->isEmpty() ? 'hidden' : '' }}">
												@foreach ($flashSale->products as $product)
													<div
														class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
						...
						<div x-show="productModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
							<div class="flex min-h-screen items-center justify-center p-4">
								<div @click="productModalOpen = false" class="fixed inset-0 bg-black/50 transition-opacity"></div>
								<div
									class="relative w-full max-w-lg rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800 shadow-xl z-50">
									...
									<div class="p-5">
										...
										<div class="max-h-80 overflow-y-auto space-y-2" id="modalProductsList">
											@forelse ($products as $product)
												@php
													$isSelected = $flashSale->products->contains($product->id);
												@endphp
												<label
													class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 cursor-pointer transition dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 searchable-product {{ $isSelected ? 'ring-2 ring-blue-500 border-blue-500' : '' }}"
													...
													<input type="checkbox" name="products[]" value="{{ $product->id }}"
														...
														{{ $isSelected ? 'checked' : '' }} onchange="handleProductCheckboxChange(this)">
													<div class="h-10 w-10 rounded-lg bg-white dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
														@if ($product->primary_image)
															<img src="{{ asset('storage/' . $product->primary_image) }}" alt="{{ $product->name }}"
																class="h-8 w-8 rounded object-cover">
		...
		<script>
			const selectedProductIds = @json($flashSale->products->pluck('id')->toArray());
					...
					if (productsList) {
						...
						checkboxes.forEach(cb => {
							...
							<div class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
								...
								<button type="button" onclick="removeProductFromFlashSale(${productId})" class="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg">
									<svg class="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
									</svg>
								</button>
