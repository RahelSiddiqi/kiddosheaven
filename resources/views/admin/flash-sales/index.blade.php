...
@section('title', 'Flash Sales — Kiddo\'s Heaven')

@section('content')
	<div class="grid grid-cols-12 gap-4 md:gap-6">
		<div class="col-span-12">
			<div x-data="flashSaleManager({
    initialSearch: '{{ request('search', '') }}',
    baseUrl: '/admin/flash-sales',
    csrf: '{{ csrf_token() }}'
})">
				<!-- Toast Notification -->
				...
				<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/3">
					...
					<div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
						...
						<div class="flex flex-col gap-3 sm:flex-row sm:items-center">
							<form @submit.prevent="searchFlashSales()">
								<div class="relative">
									<button type="button" @click="searchFlashSales()" class="absolute -translate-y-1/2 left-4 top-1/2">
									...
									<input type="text" x-model="searchTerm" @keydown="handleKeydown($event)" placeholder="Search flash sales..."
						...
						<div class="max-w-full px-5 overflow-x-auto">
							<table class="min-w-full">
								...
								<tbody class="divide-y divide-gray-200 dark:divide-gray-700">
											...
											<td class="py-4 whitespace-nowrap">
												<div class="flex items-center">
													<div class="ml-4">
														...
														<div class="text-xs text-gray-500 dark:text-gray-400">{{ $flashSale->description ?? 'No description' }}
														</div>
													</div>
												</div>
											</td>
											<td class="px-4 py-4 whitespace-nowrap">
												<div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $flashSale->discount_percentage }}%
												</div>
											</td>
											<td class="px-4 py-4 whitespace-nowrap">
												<div class="text-sm text-gray-900 dark:text-white">{{ $flashSale->starts_at->format('M d, Y H:i') }}</div>
												<div class="text-xs text-gray-500 dark:text-gray-400">to {{ $flashSale->ends_at->format('M d, Y H:i') }}
												</div>
											</td>
											<td class="px-4 py-4 whitespace-nowrap">
												<div class="text-sm text-gray-900 dark:text-white">{{ $flashSale->products->count() }} products</div>
											</td>
											<td class="px-4 py-4 whitespace-nowrap">
												{!! $flashSale->status_badge !!}
											</td>
									...
									@empty
										<tr>
											<td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No flash sales found</td>
										</tr>
									@endforelse
								</tbody>
							</table>
					...
					@if ($flashSales->hasPages())
						<div class="px-6 py-4 border-t border-gray-200 dark:border-white/5">
							<div class="flex items-center justify-between">
								<button @click="window.location.href='{{ $flashSales->appends(request()->query())->previousPageUrl() }}'"
									{{ !$flashSales->appends(request()->query())->previousPageUrl() ? 'disabled' : '' }}
									class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-3 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/3 dark:hover:text-gray-200 sm:px-3.5 {{ !$flashSales->appends(request()->query())->previousPageUrl() ? 'opacity-50 cursor-not-allowed' : '' }}">
								...
								<span class="block text-sm font-medium text-gray-700 dark:text-gray-400 sm:hidden">Page
									{{ $flashSales->currentPage() }} of {{ $flashSales->lastPage() }}</span>

								<ul class="hidden items-center gap-0.5 sm:flex">
									@foreach ($flashSales->appends(request()->query())->links()->elements[0] as $page => $url)
										<li>
											<button @click="window.location.href='{{ $url }}'"
												class="flex h-10 w-10 items-center justify-center rounded-lg text-theme-sm font-medium {{ $page == $flashSales->currentPage() ? 'bg-blue-500 text-white' : 'text-gray-700 hover:bg-blue-500/8 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-500' }}">
												{{ $page }}
											</button>
										</li>
									@endforeach
								...
								<button @click="window.location.href='{{ $flashSales->appends(request()->query())->nextPageUrl() }}'"
									{{ !$flashSales->appends(request()->query())->nextPageUrl() ? 'disabled' : '' }}
									class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-3 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/3 dark:hover:text-gray-200 sm:px-3.5 {{ !$flashSales->appends(request()->query())->nextPageUrl() ? 'opacity-50 cursor-not-allowed' : '' }}">
...
@push('scripts')
	<script>
		document.addEventListener('alpine:init', () => {
			Alpine.data('flashSaleManager', (config) => ({
				...
				handleKeydown(e) {
					if (e.key === 'Enter') this.searchFlashSales();
				},
				...
				async deleteFlashSale(id) {
					...
					try {
						...
						if (data.success) {
							this.showToast(data.message || 'Flash sale deleted successfully!');
							setTimeout(() => {
								window.location.reload();
							}, 1500);
						}
					} catch (error) {
						this.showToast('Error deleting flash sale', 'error');
					}
				},
			}))
		})
