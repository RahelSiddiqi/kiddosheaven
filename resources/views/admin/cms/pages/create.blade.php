...
@section('title', 'Create Page — Kiddo\'s Heaven')
			...
			<div class="card"
				...
				<div class="card-header" :class="{ 'dark border-slate-800': isDarkMode, 'border-gray-200': !isDarkMode }">
					...
					<p class="card-description" :class="{ 'text-gray-400': isDarkMode, 'text-gray-500': !isDarkMode }">
						Enter the basic details for this page
					</p>
				</div>
					...
					<div>
						...
						<div class="flex items-center gap-2">
							...
							<input type="text" id="slug" name="slug" value="{{ old('slug') }}"
								placeholder="auto-generated-from-title" class="input @error('slug') border-red-500 @enderror"
								:class="{ 'dark bg-slate-800 border-slate-700 text-white': isDarkMode, 'bg-white border-gray-300': !isDarkMode }">
						</div>
						<p class="text-xs mt-1" :class="{ 'text-gray-400': isDarkMode, 'text-gray-500': !isDarkMode }">
				...
				<div class="card-content space-y-4">
					...
					<div>
						...
						<textarea id="meta_description" name="meta_description" rows="3"
						 ...
						 :class="{ 'dark bg-slate-800 border-slate-700 text-white': isDarkMode, 'bg-white border-gray-300': !isDarkMode }">{{ old('meta_description') }}</textarea>
		...
		<div class="card"
			...
			<div class="card-content">
				<textarea id="content" name="content" class="wysiwyg-editor @error('content') border-red-500 @enderror"
				 :class="{ 'dark bg-slate-800 border-slate-700 text-white': isDarkMode, 'bg-white border-gray-300': !isDarkMode }">{{ old('content') }}</textarea>
				@error('content')
					<p class="text-red-500 text-sm mt-1">{{ $message }}</p>
				@enderror
			</div>
