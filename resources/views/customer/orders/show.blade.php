...
@section('title', 'Order #' . ($order->order_number ?? $order->id) . ' — Kiddo\'s Heaven')
...
@section('content')
	...
	<nav class="text-sm mb-6">
		<ol class="flex items-center gap-2 text-gray-500">
			...
			<li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg></li>
			<li><a href="{{ route('customer.orders.index') }}" class="hover:text-primary">Orders</a></li>
			<li><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg></li>
			<li class="text-gray-800 font-medium">#{{ $order->order_number ?? $order->id }}</li>
		</ol>
	</nav>
	...
	<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
		<div>
			<h1 class="text-3xl font-bold text-gray-900">Order #{{ $order->order_number ?? $order->id }}</h1>
							...
							<div class="flex-1 min-w-0">
								@if ($item->product)
									<a href="{{ route('products.show', $item->product->slug) }}"
										class="font-semibold text-gray-900 hover:text-primary">{{ $item->product->name }}</a>
								@else
									<p class="font-semibold text-gray-900">Product #{{ $item->product_id }}</p>
								@endif
								<p class="text-sm text-gray-600 mt-1">
									Qty: {{ $item->quantity }} x ৳{{ number_format($item->unit_price, 0) }}
								</p>
							</div>
							<p class="font-bold text-gray-900">৳{{ number_format($item->total_price, 0) }}</p>
						</div>
					@endforeach
				</div>
			</div>
			...
			<div class="grid md:grid-cols-2 gap-6">
				<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
					...
					<div class="space-y-2 text-sm text-gray-600">
						...
						<p><span class="font-medium text-gray-900">Email:</span> {{ $order->customer_email ?? 'N/A' }}</p>
			...
			<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
				...
				<div class="space-y-3 mb-6">
					<div class="flex justify-between text-gray-600">
						<span>Subtotal</span>
						<span class="font-semibold">৳{{ number_format($order->subtotal ?? $order->total_amount, 0) }}</span>
					</div>
					<div class="flex justify-between text-gray-600">
						<span>Tax</span>
						<span class="font-semibold">৳{{ number_format($order->tax_amount ?? 0, 0) }}</span>
					</div>
					<div class="flex justify-between text-gray-600">
						<span>Shipping</span>
				...
				<div class="border-t border-gray-200 pt-4 mb-6">
					<div class="flex justify-between items-center">
						...
						<span class="text-2xl font-bold text-primary-dark">৳{{ number_format($order->total_amount, 0) }}</span>
					</div>
				</div>
